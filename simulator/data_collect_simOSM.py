...
class mgl_imgui_simOSM(mglw.WindowConfig):
    ...
    def get_view2particleidx(
        ...
        # depth = 2*self.far*self.near / (self.far + self.near - (self.far - self.near)*(2*lin_depth-1));
...
def main(map_name):
    ...
    # positions = particles[0::80,:]
    npx = 10
    npy = 10
    x = np.linspace(-100, 100, npx)
    y = np.linspace(-100, 100, npy)
    Py, Px = np.meshgrid(y, x)
    z = np.ones(int(npx * npy))

    ## transpose breaks c-contiguous memory , copy fixes that issue
    poses = np.vstack((Px.flatten(), Py.flatten(), z)).T.copy(order="C")

    # positions = poses[np.random.randint(0,8000,100),:]
    #params for nyc_stuy
    positions = poses#[::20,:]
    positions[:,2] = 125.
    ...
    for pos in positions:
        image, depth, T = app.run_view2particles(
            pos, 270, -90, [], True
        )
        images.append(image)
        depths.append(depth)
    ...
    save_dir = './data/stuy_lin/'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    pickle.dump(data, open(save_dir+"philly_cc.pkl", "wb"))
    cam_poses = []
