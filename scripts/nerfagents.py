...
class scout:
    ...
    def hallucinate(sf, x):
        ...
        for ii in range(Nx):
            ...
            for _ in range(4):
                if sf.cfg["mode"] == "nerf":
                    points = sf.sim.world2pixel(
                        sf.pf.p_targ_og, pos=x[ii, :3], yaw=yaw, pitch=x[ii, 4]
                    )
                    valid_points = sf.nerf.particles_seen_from_pose(points, x[ii, :3])
                    part_idx = sf.world2particleidx(valid_points,sf.pf.p)
    ...
    def step(sf, nerf):
        ...
        for ii in range(xp.shape[0]):
            uncertainties.append(sf.nerf.probablistic_uncertainty(xp[ii, :]))

        # sum preditive informations across [rgb, depth, occ]
        unc = np.sum(np.array(uncertainties), axis=1)
        ...
        mi = np.tile(unc, (sf.pf.num_targets + 1, 1)).T + I*λ
        # print("mi", mi)

        ## based on samples across targets create multinomial distribution
        samp_idx = np.zeros(mi.shape[1]).astype('int')
        sampI = np.zeros(mi.shape[1])
        for ii in range(mi.shape[1]):
            mi_prob = np.exp(mi[:,ii])/ np.sum(np.exp(mi[:,ii]))
            samp_idx[ii] = np.nonzero(np.random.multinomial(1, mi_prob))[0][0]
            sampI[ii] = mi[samp_idx[ii],ii]
        ...
        return traj, uncertainties, I, chosen_idx
