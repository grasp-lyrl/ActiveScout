...
from nerf_scripts.nerf_predictive_info import ActiveNeRFMapper as nerf
...
class pipeline:
    def __init__(sf, seed, cfg, mapDim):
        ...
        sf.nerf = nerf(sf.cfg)
        if not os.path.exists(sf.nerf.save_path):
            os.makedirs(sf.nerf.save_path)
    ...
    def warmup_nerf(sf):
        ...
        """
        # test_idx_pos = np.random.randint(0,sf.agents[0].pf.N,20)
        # test_data = {"images": [], "depths":[], "poses":[]}
        # for idx in test_idx_pos:
        #     _, _, _, real_data = sf.agents[0].sim.run_view2particles(
        #             np.array([sf.agents[0].pf.p[idx, 0], sf.agents[0].pf.p[idx, 1], 200.]),
        #             yaw=-90.,
        #             pitch=-90.,
        #             targ_pos=[],
        #             in_situ=True,
        #             save_img=-1,
        #         )
        #     test_data["images"].append(real_data["image"])
        #     test_data["depths"].append(real_data["depth"])
        #     test_data["poses"].append(real_data["pose"])

        for ii in range(sf.num_agents):
            traj = sf.agents[ii].survey_traj()
            data = sf.rollout(ii, traj)

        sf.nerf.initialization(data, data)
        sf.nerf.nerf_training(
            steps=sf.cfg["warmup_train_steps"], initial_train=True, planning_step=-1
        )
    ...
    def step(sf):
        ...
        for ii in range(sf.num_agents):
            ...
            traj, unc, I, chosen_idx = sf.agents[ii].step(sf.nerf)
            data = sf.rollout(ii, traj)
            ...
            sf.nerf.train_dataset.update_data(
                np.array(data["images"]),
                np.array(data["depths"]),
                np.array(data["poses"]),
            )

            sf.nerf.test_dataset.update_data(
                np.array(data["images"]),
                np.array(data["depths"]),
                np.array(data["poses"]),
            )
        ...
        if sf.cfg["offline_mode"] == 1:
            if sf.step_count <= 40:
                sf.nerf.nerf_training(
                    steps=sf.cfg["inbtwn_train_steps"], planning_step=sf.step_count
                )
            else:
                sf.nerf.nerf_training(steps=2000, planning_step=sf.step_count)
        else:
            sf.nerf.nerf_training(
                steps=sf.cfg["inbtwn_train_steps"], planning_step=sf.step_count
            )
        sf.nerf.render(data)
        # sf.nerf.render(sf.test_data, mode="test/")

        if sf.step_count == sf.cfg["num_steps"] - 1:
            sf.nerf.nerf_training(
                steps=4000, final_train=True, planning_step=sf.step_count
            )
            sf.nerf.render(data)
            # sf.nerf.render(sf.test_data, mode="test/")
    ...
    def rollout(sf, ii, traj):
        ...
        for jj in range(sf.num_targets):
            ...
            ## used for the offline nerf example
            if sf.cfg["offline_mode"] == 1:
                if sf.step_count <= 40:
                    sf.targets[jj].mode = "stationary"
                else:
                    sf.targets[jj].mode = "active"
    ...
    def reward(sf):
        ...
        pickle.dump(sf.metric, open(sf.nerf.save_path + "/metrics.pkl", "wb"))
    ...
    def render(sf):
        ...
        for jj in range(sf.num_targets):
            ax1.plot(
                sf.metric["targ-%d" % jj], label="targ-%d" % jj
            )  # , color=colors[jj])
        ax1.legend()
        ...
        sf.fig.savefig(sf.nerf.save_path + "/2dplot.pdf", bbox_inches="tight")
        sf.fig_metric.savefig(sf.nerf.save_path + "/mse.pdf", bbox_inches="tight")
        pickle.dump(sf.a_traj, open(sf.nerf.save_path + "/a_traj.pkl", "wb"))
        pickle.dump(sf.t_traj, open(sf.nerf.save_path + "/t_traj.pkl", "wb"))
...
@click.option("--offline_mode", default=0, type=int)
def main(seed, map_name, num_steps, target_mode, offline_mode):
    cfg, mapDim = init_env(map_name)
    cfg["seed"] = seed
    cfg["num_steps"] = num_steps
    cfg["target_mode"] = target_mode
    cfg["offline_mode"] = offline_mode
    if cfg["offline_mode"] == 1:
        num_steps = 80
