...
class Dataset(torch.utils.data.Dataset):
    ...
    def render_probablistic_image_from_pose(
        ...
        # this is the case when we just give 1 pose rather than a traj of poses
        if poses.ndim == 1:
        ...
        for i, p in enumerate(poses):
            # pose = [x,y,z,cam_yaw,cam_pitch]
            # pose in rotation matrix form 4x4 from euler
            r = R.from_euler('xyz', [p[4], -p[3], 0], degrees=True).as_matrix()
            T = np.eye(4)
            T[:3,:3] = r
            T[0,3] = p[0]
            T[1,3] = p[2]
            ...
            pose = torch.from_numpy(T).unsqueeze(0).float().to(device)
